...
class softmax_classifier:
    ...
    def build_classifer(self):
        ...
        self.output_layer = lasagne.layers.DenseLayer(input_layer, num_units=10, W=lasagne.init.HeNormal(1.0),
        ...
        momentum = 0.9
        post_updates = lasagne.updates.nesterov_momentum(train_loss, self.params, learning_rate=LR, momentum=momentum)
...
def train_classifer(network, datasets, log_path, snn_loaded_object):
    ...
    # LR_list = [10.0, 1.0, 0.1, 0.1, 0.01, 0.001, 0.0001, 0.000]
    LR_list = [0.00001,0.00001, 0.00001, 0.00001, 0.000001, 0.000001, 0.0000001, 0.0000001]
    ...
    for epoch in range(num_epochs):
        ...
        for i, (X, Y) in enumerate(datasets['train']):
            ...
            # output_layer = lasagne.layers.DenseLayer(input_layer, num_units=10, W=lasagne.init.HeNormal(1.0),
            #                                               nonlinearity=lasagne.nonlinearities.softmax)
            #
    ...
    #  _, txt = test_classifier(network, datasets['test'], snn_loaded_object)
...
def train_classifer_main():
    ...
    path = os.path.join(model_save_path, 'classifer_n_momentum')
