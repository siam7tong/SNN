...
from theano.sandbox.cuda.basic_ops import as_cuda_ndarray_variable, as_cuda_ndarray_constant, gpu_contiguous
...
class stdpOp(GpuOp):
    ...
    def make_node(self, output_spike, H_out, weights):
        ...
        if output_spike.type.ndim != 4:  # conv1 (1,30,28,28)
        ...
        output_spike = as_cuda_ndarray_variable(output_spike,)  # 我以为，程序在这里根据输入数据的特征将其转换为
    ...
    def c_code(self, node, nodename, inp, out_, sub):
        ...
        inp 是一系列字符串，作为op的输入，每个字符串都包含了与输入一致的C变量名字，C变量的名字inp[0]代表了给的第一个输入，
        要用这个名字去与这个变量联系。out_ 与inp意义一致。
