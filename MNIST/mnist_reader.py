...
class mnist_data_set:
    ...
    def __init__(self, data_path, batch_size=1, valid_set_size_percent=0.1):
        random.seed(11)
        self.data_path = data_path
        X_train, Y_train, X_test, Y_test = self.load_mnist()
        # self.X_mean = np.mean(X_train, axis = 0)
        valid_set_size = int(np.ceil(X_train.shape[0] * valid_set_size_percent))
        X_train, Y_train, X_valid, Y_valid = self.create_validation_set(X_train, Y_train, valid_set_size)
        # self.X_mean = np.mean(X_train, axis = 0)
        # X_train -= self.X_mean
        # X_valid -= self.X_mean
        # X_test -= self.X_mean
        # print 'Mean subtracted dataset.', 下面的data_sets是类属性，每个实例也有
        random_train_size = np.int_(12000)

        X_train, Y_train = self.create_random_train_set(X_train, Y_train, random_train_size)
        self.data_sets = {
    ...
    def load_mnist_images(self, filename):
        with gzip.open(filename, 'rb') as f:
            data = np.frombuffer(f.read(), np.uint8, offset=16)  # from buffer make np
            data = data.reshape(-1, 1, 28, 28)  # -1表示行数我们不知道或懒得填,让系统自己算, 表示通道应该多少
            # The inputs come as bytes, we convert them to float32 in range [0,1]. 正则化
    ...
    def create_random_train_set(self, X, Y, random_train_size):
        np.random.seed(10)
        random_select_index = np.random.randint(0, 54000, random_train_size)

        # slice =random.sample(X, random_train_size)

        return X[random_select_index], Y[random_select_index]
